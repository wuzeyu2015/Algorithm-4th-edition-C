#ifndef INC_06_QUICK_SORT_DEAL_WITH_NEARLY_ORDERED_ARRAY_SORTTESTHELPER_H
#define INC_06_QUICK_SORT_DEAL_WITH_NEARLY_ORDERED_ARRAY_SORTTESTHELPER_H

#include <iostream>
#include <algorithm>
#include <string>
#include <ctime>
#include <cassert>

using namespace std;


namespace SortTestHelper {

	int *generateRandomArray(int n, int range_l, int range_r) {

		int *arr = new int[n];

		srand(time(NULL));
		for (int i = 0; i < n; i++)
			arr[i] = rand() % (range_r - range_l + 1) + range_l;
		return arr;
	}

	int *generateNearlyOrderedArray(int n, int swapTimes){

		int *arr = new int[n];
		for(int i = 0 ; i < n ; i ++ )
			arr[i] = i;

		srand(time(NULL));
		for( int i = 0 ; i < swapTimes ; i ++ ){
			int posx = rand()%n;
			int posy = rand()%n;
			swap( arr[posx] , arr[posy] );
		}

		return arr;
	}

	int *copyIntArray(int a[], int n){

		int *arr = new int[n];
		copy(a, a+n, arr);
		return arr;
	}

	void printArray(int arr[], int n) {

		for (int i = 0; i < n; i++)
			cout << arr[i] << " ";
		cout << endl;

		return;
	}

	void printArray(int arr[], int l, int r) {

		for (int i = l; i <= r; i++)
			cout << arr[i] << " ";
		cout << endl;

		return;
	}

	bool isSorted(int arr[], int n) {

		for (int i = 0; i < n - 1; i++)
			if (arr[i] > arr[i + 1])
				return false;

		return true;
	}

	bool isPartitionSorted(int arr[], int l, int r) {

		for (int i = l; i < r; i++)
			if (arr[i] > arr[i + 1])
				return false;

		return true;
	}

	void testSort(const string &sortName, void (*sort)(int[], int), int arr[], int n) {

		clock_t startTime = clock();
		sort(arr, n);
		clock_t endTime = clock();
		cout << sortName << " : " << double(endTime - startTime) / CLOCKS_PER_SEC << " s"<<endl;

		assert(isSorted(arr, n));

		return;
	}

	void testPartitionSort(const string &sortName, void (*sort)(int[], int, int), int arr[], int l, int r) {

		clock_t startTime = clock();
		sort(arr, l, r);
		clock_t endTime = clock();
		cout << sortName << " : " << double(endTime - startTime) / CLOCKS_PER_SEC << " s"<<endl;

		assert(isPartitionSorted(arr, l, r));

		return;
	}

};

#endif //INC_06_QUICK_SORT_DEAL_WITH_NEARLY_ORDERED_ARRAY_SORTTESTHELPER_H
